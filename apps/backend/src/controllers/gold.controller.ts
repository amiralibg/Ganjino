...
import { fetchGoldPrices, get18KGoldPrice, getUSDPrice } from '../services/goldPrice.service';
...
export const getUSD = async (_req: Request, res: Response): Promise<void> => {
  try {
    const price = await getUSDPrice();
    res.status(200).json({ price, unit: 'تومان' });
  } catch (error: unknown) {
    console.error('GetUSD error:', error);
    res.status(500).json({ error: MESSAGES.gold.failedFetchUsdPrice });
  }
};
